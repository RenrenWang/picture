...
router.get("/s/ok/:page", function (req, res, next) {
  ...
  imgs.findAllOk(
    ...
    pageSize,
    function (err, result) {
      if (err) return res.json({ status: 0, error: `获取失败: ${err}` });
...
router.post("/add/text", function (req, res, next) {
  ...
  imgs.add(data, function (err, result) {
    if (err) return res.json({ status: 0, error: `保存失败:${err}` });
...
router.post("/updateimg", function (req, res, next) {
  ...
  imgs.update(
    { description: description, kind: kind, open: open, id: id },
    function (err, result) {
