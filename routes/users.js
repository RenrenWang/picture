...
router.post('/login', function(req, res, next) {
	...
	user.usernameFind(username, function(err, result) {
		...
		user.updateLastTime({ id: id, last_time: new Date().getTime() }, function(err, result) {
			if(err) return res.json({ "status": 0, "error": "登录失败！！" });
			console.log("执行执行");
...
router.post('/register', function(req, res, next) {
	...
	user.usernameFind(username, function(err, result) {
		...
		user.add([username, _util.setMd5(password), create_time,type], function(err, result) {
...
router.all('/updateAvatar',function(req,res,next){
	...
	form.parse(req, function(err, fields, files) {
		...
		var newPath = __dirname + '/../avatar/' + _util.setMd5(req.session.username + '_' + (new Date().getTime())) + path.extname(files.file.name);
